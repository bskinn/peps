...
Times have changed. With the introduction of the cheese shop (PyPI), setuptools,
...
* Modules in the standard library are generally favored and seen as the
  ...
  using lxml instead of `xml`. The removal of an unmaintained stdlib module
...
* A lean and mean standard library benefits platforms with limited resources
  ...
  (e.g. pyodide) also benefits from reduced download size.
...
and miscellaneous modules. The majority of modules are for old data formats or
...
The `uu <https://docs.python.org/3/library/uu.html>`_ module provides the
...
support for reading and writing Electronic Arts's Interchange File Format.
...
support for newer sound devices and improvements were proprietary. The Linux
...
The *wave* module is one of the three modules that depend on `audioop`_
(`aifc`_ and `sunau`_ are the others). With deprecation of the other
multimedia formats, it would be the last multimedia module.
...
Name Service Switch, LDAP, and Kerberos/GSSAPI have been considered more powerful
and more secure replacements for NIS.
...
direct access to the Unix shadow password database using non-standard APIs.
In general it's a bad idea to use the spwd. The spwd circumvents system
security policies, does not use the PAM stack, and is
...
functions have been deprecated since Python 3.3 and the module itself since
Python 3.4.
...
module is used to facilitate distutils's creation of MSI installers with the
...
which re-implements the ``_crypt`` extension in a few lines of ctypes code.
...
* Brett Cannon recommended to delay active deprecation warnings and removal
  ...
  shortly before Python 2 reaches end of lifetime. A delay should provide
